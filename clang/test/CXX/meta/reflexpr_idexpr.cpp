...
int main() {
  ...
  check_expr_splice_dependent_val<reflexpr(global1), 75>();

  static_assert([<reflexpr(A)>] == 0);
  check_expr_splice_dependent_val<reflexpr(A), 0>();

  static_assert([<reflexpr(S::value)>] == 4);
  check_expr_splice_dependent_val<reflexpr(S::value), 4>();

  static_assert([<reflexpr(f)>] == &f);
  check_expr_splice_dependent_val<reflexpr(f), &f>();

  static_assert([<reflexpr(f)>] == f);
  check_expr_splice_dependent_val<reflexpr(f), f>();

  static_assert(&[<reflexpr(global0)>] == &global0);
  check_expr_splice_dependent_addr<reflexpr(global0), &global0>();

  static_assert(&[<reflexpr(S::value)>] == &S::value);
  check_expr_splice_dependent_addr<reflexpr(S::value), &S::value>();
